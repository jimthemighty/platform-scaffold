...
test('renders extra content correctly', () => {
    ...
    const primaryWrapper = wrapper.find('.amp-list-tile__primary')
    ...
    expect(primaryWrapper.find('.amp-list-tile__content').children().length).toBe(1)
    expect(primaryWrapper.find('.amp-list-tile__content').childAt(0).type()).toBe('span')
    expect(primaryWrapper.find('.amp-list-tile__content').childAt(0).text()).toBe('override content')
})
...
test('does not render a childIcon if hasChild is false', () => {
    ...
    expect(wrapper.find('.amp-list-tile').hasClass('amp--has-child')).toBe(false)
    expect(primaryWrapper.children().length).toBe(1)
    expect(primaryWrapper.childAt(0).hasClass('amp-list-tile__content')).toBe(true)
})

test('includes the component class name with no className prop', () => {
    const wrapper = shallow(<NavItem />)
    expect(wrapper.hasClass('amp-nav-item')).toBe(true)
})
