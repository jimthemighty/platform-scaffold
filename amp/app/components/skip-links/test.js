...
const invalidPropError = 'SkipLinks item target is invalid. Must be an ID selector, starting with "#" followed by alphanumeric characters, dashes and underscores with no spaces.'
const itemsStub = [{
    target: '#test1',
    label: 'Test 1',
}, {
...
test('fails propType validation if target does not start with "#"', () => {
    expect(
        targetPropType([
            {target: 'theCharacterShouldBeAtStart', label: ''}
        ])
    ).toEqual(new Error(invalidPropError))
})

test('fails propType validation if target has spaces', () => {
    expect(
        targetPropType([
            {target: '#no spaces allowed', label: ''}
        ])
    ).toEqual(new Error(invalidPropError))
})
...
test('fails propType validation if target has non-alphanumeric characters', () => {
    expect(
        targetPropType([
            {target: '#invalid@string.com', label: ''}
        ])
    ).toEqual(new Error(invalidPropError))
})
