...
const submitForm = (href, formValues, formSelector, responseUrl) => {
    ...
    return makeFormEncodedRequest(href, formValues, {method: 'POST'})
        ...
        .then((res) => {
            ...
            const magentoCacheStorage = isLocalStorageAvailable() ? JSON.parse(localStorage.getItem('mage-cache-storage')) : decodeURIComponent(getCookieValue('ls-mage-cache-storage'))
            ...
            if (isLocalStorageAvailable()) {
                ...
                document.cookie = `ls-mage-cache-storage=${encodeURIComponent(magentoCacheStorage)}; expires=0 UTC; path=/`
            }
